...
class CompetitionsIdCategoriesIdWinner final : public userver::server::handlers::HttpHandlerBase {
private:
...
public:
	static constexpr std::string_view kName = "handler-competitions-id-categories-id-winner";

	CompetitionsIdCategoriesIdWinner(
	...
	std::string HandleRequestThrow(
		...
		LOG_WARNING() << "CompetitionsIdCategoriesIdWinner: start";
		...
		const auto winner = participantRepository->getWinnerCategory(competitionId, categoryId);
		if (!winner.has_value()) {
            response.SetStatus(userver::server::http::HttpStatus::kNotFound);
			return {};
        }
        ...
        return userver::formats::json::ToString(
            application::mappers::competitions::json::toJson(winner.value())
        );
	}
};
...
void AppendCompetitionsIdCategoriesIdWinner(userver::components::ComponentList &component_list) {
	component_list.Append<CompetitionsIdCategoriesIdWinner>();
}
