...
class CompetitionsIdRegister final : public userver::server::handlers::HttpHandlerBase {
private:
...
public:
	static constexpr std::string_view kName = "handler-competitions-id-register";

	CompetitionsIdRegister(
	...
	std::string HandleRequestThrow(
		...
		LOG_WARNING() << "CompetitionsIdRegister: start";
		...
		if (competition.value().status != domain::competitions::kRegistration) {
			response.SetStatus(userver::server::http::HttpStatus::kBadRequest);
			return {};
		}

		const auto bodyJson = userver::formats::json::FromString(request.RequestBody());
    	auto participantRegisterCommand = mappers::participants::request::getParticipantRegisterCommand(bodyJson);

		participantRepository->registerParticipant(participantRegisterCommand, competitionId, claims.userId);
...
void AppendCompetitionsIdRegister(userver::components::ComponentList &component_list) {
	component_list.Append<CompetitionsIdRegister>();
}
